...
const categories = ['すべて', '市場動向', '法改正・制度', '制度・補助金', 'テクノロジー', '経営戦略', '業界トピックス', '統計レポート'];
...
export default function NewsPage() {
  ...
  const [articles, setArticles] = useState<BizNews[]>([]);
  ...
  useEffect(() => {
    fetch('/api/biz-news')
      .then((r) => r.json())
      .then((data) => {
        const list = Array.isArray(data) ? (data as BizNews[]) : [];
        setArticles(list.filter((n) => !n.status || n.status === 'published'));
      })
      .catch(() => setArticles([]))
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          {loading ? (
            ...
            <>
              ...
              {paged.length === 0 && (
                <div className="text-center py-12 text-gray-400 text-sm">該当するニュースがありません</div>
              )}
