...
import { useBizNews, bizNewsStore } from '@/lib/content-store';
import { type BizNewsItem } from '@/lib/biz-news-data';

type EditMode = 'list' | 'add' | 'edit';

export default function BizNewsAdmin() {
  const [mode, setMode] = useState<EditMode>('list');
  const [editItem, setEditItem] = useState<BizNewsItem | null>(null);
  const items = useBizNews();
  ...
  const columns = [
    ...
    { key: 'category', label: 'カテゴリー', render: (v: unknown) => {
      const colors: Record<string, string> = {
        '市場動向': 'bg-blue-100 text-blue-700',
        '法改正・制度': 'bg-green-100 text-green-700',
        'テクノロジー': 'bg-purple-100 text-purple-700',
        '経営戦略': 'bg-amber-100 text-amber-700',
      };
  ...
  const handleDelete = (item: Record<string, unknown>) => {
    bizNewsStore.set((prev) => prev.filter((p) => p.id !== item.id));
  };
      ...
      <BizNewsForm
        ...
        onSave={(data) => {
          if (mode === 'edit' && editItem) {
            bizNewsStore.set((prev) => prev.map((p) => (p.id === editItem.id ? { ...p, ...data } : p)));
          } else {
            ...
            bizNewsStore.set((prev) => [data as BizNewsItem, ...prev]);
          }
          setMode('list');
          setEditItem(null);
        }}
  ...
  return (
    <ContentTable
      title="Bizニュース管理"
      description="工務店・ハウスメーカー向けニュースを管理します"
      columns={columns}
      data={items as unknown as Record<string, unknown>[]}
      onAdd={() => setMode('add')}
      onEdit={(item) => { setEditItem(item as unknown as BizNewsItem); setMode('edit'); }}
      onDelete={handleDelete}
    />
  );
}

function BizNewsForm({ item, onSave, onCancel }: { item: BizNewsItem | null; onSave: (data: Partial<BizNewsItem>) => void; onCancel: () => void }) {
  const [form, setForm] = useState({
    id: item?.id ?? '',
    title: item?.title ?? '',
    date: item?.date ?? '',
    category: item?.category ?? '市場動向',
  ...
  const categories = ['市場動向', '法改正・制度', 'テクノロジー', '経営戦略'];

  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onCancel} className="text-sm text-gray-500 hover:text-[#E8740C] cursor-pointer">← 戻る</button>
        <h1 className="text-2xl font-bold text-gray-900">{item ? 'Bizニュースを編集' : 'Bizニュースを新規追加'}</h1>
      </div>
