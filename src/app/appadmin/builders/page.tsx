...
export default function BuildersAdmin() {
  ...
  const [items, setItems] = useState<Builder[]>(INITIAL_BUILDERS);
      ...
      <BuilderForm
        ...
        onSave={(data) => {
          if (mode === 'edit' && editItem) {
            ...
            setItems((prev) => [data as Builder, ...prev]);
          }
          setMode('list');
          setEditItem(null);
        }}
  ...
  return (
    <ContentTable
      ...
      onAdd={() => setMode('add')}
      onEdit={(item) => { setEditItem(item as unknown as Builder); setMode('edit'); }}
      onDelete={handleDelete}
    />
  );
}

function BuilderForm({ item, onSave, onCancel }: { item: Builder | null; onSave: (data: Partial<Builder>) => void; onCancel: () => void }) {
