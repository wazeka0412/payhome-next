...
export type FieldType = 'text' | 'textarea' | 'longtext' | 'number' | 'select' | 'tags' | 'date' | 'image'
...
export interface FieldDef {
  ...
  type: FieldType
  options?: string[]
  /** Show in the table list view */
  inList?: boolean
  /** Hide from the edit form */
  hideInForm?: boolean
  /** Render hint for table cell */
  format?: (v: unknown, row: Record<string, unknown>) => string
  /** Width hint for the table column (Tailwind) */
  className?: string
  placeholder?: string
}

interface Props {
  /** REST endpoint base, e.g. "/api/articles" */
  endpoint: string
  /** Page title */
  title: string
  /** Field definitions used for both the table and the edit form */
  fields: FieldDef[]
  /** Optional default values when creating a new row */
  newDefaults?: Record<string, unknown>
  /** Search keys (defaults to "title") */
  searchKeys?: string[]
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">全 {rows.length} 件（表示中: {filtered.length} 件）</p>
        </div>
      ...
      {creating && (
        <div className="bg-white rounded-xl shadow-sm border border-orange-200 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">新規作成</h2>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button
              ...
              disabled={saving}
              className="px-4 py-2 text-sm bg-[#E8740C] text-white rounded-lg hover:bg-[#D4660A] disabled:opacity-50 cursor-pointer"
            >
              {saving ? '保存中...' : '作成する'}
            </button>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-right py-3 px-4 text-gray-500 font-medium w-32">操作</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((row) => {
                    ...
                    <tr className="border-b border-gray-50 hover:bg-orange-50/50 transition">
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          type="button"
                          onClick={() => isEditing ? cancelEdit() : startEdit(row)}
                          className="text-xs text-[#E8740C] font-medium mr-3 hover:underline cursor-pointer"
                        >
                          {isEditing ? '閉じる' : '編集'}
                        </button>
                        <button
                      ...
                      <tr className="bg-red-50/40">
                        <td colSpan={listFields.length + 2} className="px-6 py-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-red-700">本当にこの項目を削除しますか？</span>
                            <div className="flex gap-2">
                              <button
                                type="button"
                                onClick={() => setConfirmDeleteId(null)}
                                className="px-3 py-1.5 text-xs text-gray-600 border border-gray-200 rounded-lg hover:bg-white cursor-pointer"
                              >
                                キャンセル
                              </button>
                              <button
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center text-gray-400 text-sm">該当するデータがありません</div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {fields.map((f) => {
        ...
        const baseClass =
          'w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8740C]/30 focus:border-[#E8740C]'
        return (
          <div key={f.key} className={f.type === 'longtext' || f.type === 'textarea' ? 'md:col-span-2' : ''}>
            ...
            {f.type === 'date' && (
              <input type="text" value={String(value)} placeholder="YYYY-MM-DD or YYYY.MM.DD" onChange={(e) => set(f.key, e.target.value)} className={baseClass} />
            )}
            {f.type === 'number' && (
            ...
            {f.type === 'longtext' && (
              <textarea value={String(value)} rows={8} placeholder={f.placeholder} onChange={(e) => set(f.key, e.target.value)} className={`${baseClass} resize-y font-mono text-xs`} />
            )}
            {f.type === 'select' && (
