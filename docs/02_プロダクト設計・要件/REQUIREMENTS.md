...
> **v4.1 パッチ (2026-04-12):**
> - **資料請求は主要導線・主要KPIから完全に除外** (残存していた catalog_request イベント、recommended_action、チャット案内記述をすべて削除)
> - **比較機能 (`/builders/compare`) は MVP 非公開** に変更 (middleware で 404)。会員価値訴求からも削除
> - **v4.1 の MVP 主要価値**: (a) 間取り図フル解像度、(b) AI 診断結果保存、(c) お気に入り整理 (5 カテゴリ)、(d) 事例ライブラリ全件閲覧、(e) 連絡希望条件の事前設定 (Smart Match)
> - 成果報酬 3% の対象は「無料相談 / AI 診断 / 会員登録済みユーザー」経由の成約に限定 (Phase 3〜)
>
> **v4.0 での主要変更（2026-04-02）:**
> - 資料請求中心モデルから、会員登録 × AI診断 × 見学会予約の高品質送客モデルへ転換
> - F-07「まとめて資料請求」を廃止
> - MVP画面を 7画面 + 会員機能 に絞る（月刊 / ウェビナー / ニュース / コラムはPhase 2以降）
> - 会員登録で解放される価値 を新章として追加
> - AI機能を 5機能 に分解して定義
...
| + | 平屋事例ライブラリ | /case-studies, /case-studies/[id] | 完成事例を間取り/費用/工務店/タグで検索。非会員は5件まで、6件目以降は会員登録必須 | ✅ |
...
- `/catalog`（デジタルカタログ閲覧）※2026-04-12 修正: 資料請求の概念は廃止。カタログ閲覧のみ残存、補助導線扱い
...
| recommended_action | text | 推薦アクション（見学会予約/無料相談/AI診断/会員登録）※2026-04-12: 資料請求を候補から除外 |
...
| 相談導線接続 | 会話の中で AI診断・無料相談・見学会予約・会員登録 を案内 | CTA用のリンクをプロンプトに含める (2026-04-12: 資料請求案内は削除) |
...
| catalog_view | デジタルカタログ閲覧開始 | GA4 + user_events | Phase 1 (※旧: catalog_request は 2026-04-12 で廃止) |
...
| A | AIリードスコアリング（静的→動的） |
| B | ~~平屋事例ライブラリ MVP~~ → **MVP に前倒しして実装済（非会員5件ゲート付き）** |
| B | 家づくりコミュニティ MVP |
| B | GA4カスタムイベント実装 |
| B | 閲覧履歴ページのライブデータ接続 |
| B | AI会話要約・意図抽出バッチ |
