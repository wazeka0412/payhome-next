...
## 収益ドリブン機能要件（2026-03-26追加）

### まとめて資料請求（F-07）
- ユーザーが工務店一覧または比較画面で最大3社を選択
- 1回のフォーム送信で選択した全社に資料請求を送信
- 各工務店にリード通知メールを個別送信
- leadsテーブルに工務店ごとに1レコードずつ作成
- source_typeは「bulk_catalog_request」とする

### 見学会残席表示（F-08）
- eventsデータにcapacity（定員）とreserved_count（予約数）を追加
- 残席数 = capacity - reserved_count を算出し表示
- 残席3以下で「残りわずか」のバッジを表示
- 残席0で「満席」表示＋予約ボタン無効化
- トップページに「今週末の見学会」セクションを追加

### 工務店比較機能（F-09）
- ユーザーが2〜3社の工務店を選択して比較表を表示
- 比較項目：坪単価・対応エリア・平屋実績・デザインテイスト・性能
- 比較画面から「まとめて資料請求」ボタンを表示
- comparison_historyイベントをuser_eventsに記録
